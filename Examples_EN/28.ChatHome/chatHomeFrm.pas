...
uses
  ...
  Z.ListEngine, Z.UnicodeMixedLib, Z.MemoryStream, Z.Expression, Z.OpCode, Z.Net.XNAT.Physics, Z.Notify,
  Z.Net, Z.Net.DoubleTunnelIO.NoAuth,
  FMX.Memo.Types;

type
  {  The server creates instances for each user,  }
  {  Customized user storage information is stored here  }
...
begin
  {  In dual channel programming, a linkOK judgment should be added before the instruction  }
  {  Linkok also indicates successful verification and only allows users who have successfully verified to speak  }
...
begin
  {  In anonymous functions, the stack is unstable, and the use of data requires either container variables, global variables, or replication  }
