...
uses SysUtils,
{$IFDEF FPC}
  Z.FPC.GenericList, {  To enable the fpc compiler to support generics, this library is required  }
{$ENDIF FPC}
  Z.Core, Z.PascalStrings, Z.Parsing, Z.UnicodeMixedLib, Z.Status, Z.ListEngine;
...
{$IFDEF FPC}
  ...
  {  The fpc compiler needs to use the specialize modifier before declaring generics  }
...
{$ENDIF FPC}
  ...
  public
    ...
    {  These three places require the release of members. If Notify is not used, it is in a generic library that is compatible with FPC: FPCGenericStructlist.pas library does not provide Notify callback  }
...
begin
  ...
  DoStatus('After sorting dependencies');
  for i := 0 to pl.Count - 1 do
      DoStatus(pl[i].Unit_Name);
