...
begin
  ...
  for i := 1 to 10 do
    begin
      ...
      {  Push will recopy tmp to a memory block  }
  ...
  if false then
    begin
      {  There are two ways to obtain data: 1. Directly determine whether the current is nil  }
      while order.Current <> nil do
        begin
          with order.Current^.data^ do
              DoStatus('a:%d, b:%d, c:%d', [a, b, c]);

          order.Next; {  Next will release the current structure, while also releasing memory and pointing to the next structure  }
        end;
    end
  else
    begin
      {  Two ways to obtain data, 2. Determine the current queue num  }
