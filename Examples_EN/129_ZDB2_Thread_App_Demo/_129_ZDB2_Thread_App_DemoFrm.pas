...
type
  ...
  private
    procedure backcall_DoStatus(Text_: SystemString; const ID: Integer); {  Status log  }
    procedure Do_Init_Sim_Mem_DB;                                        {  Initialize db  }
    procedure Do_Sim_Add;                                                {  Multi thread additional data simulation  }
    procedure Do_Sim_Remove;                                             {  Multi thread deletion data simulation  }
    procedure Do_Sim_Parallel_Load;                                      {  Multi threaded physical traversal simulation  }
    procedure Do_Sim_For;                                                {  Multi threaded instance traversal simulation  }
  public
...
begin
  CheckThread;
  Data_Num_Label.Caption := Format('Simulation task:%d Data link collector:%d Data instance collector:%d Total number of data entries:%d',
    [Task_Num, Sim_Mem_DB.Data_Link_Recycle_Tool.Num, Sim_Mem_DB.Instance_Recycle_Tool.Num, Sim_Mem_DB.Total]);
  Add_Num_Label.Caption := Format('Number of times data was added:%d', [Add_Num]);
  Remove_Num_Label.Caption := Format('Delete data count:%d', [Remove_Num]);
  Parallel_Load_Num_Label.Caption := Format('Physical traversal count:%d', [Parallel_Load_Num]);
  For_Num_Label.Caption := Format('Instance traversal count:%d', [For_Num]);
  Progress_Num_Label.Caption := Format('Number of main cycles:%d', [Progress_Num]);
    ...
    with Sim_Mem_DB.Engine_Pool.Repeat_ do
      repeat
        if Queue^.Data.Engine <> nil then
            n.Append('Database (%d) IO read/write queue:%d Storage capacity:%s/%s' + #13#10,
            [I__ + 1,
