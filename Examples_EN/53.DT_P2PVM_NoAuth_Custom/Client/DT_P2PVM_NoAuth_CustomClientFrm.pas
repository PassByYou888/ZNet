  ...
  PhysicsTunnel.OnAutomatedP2PVMClientConnectionDone_P := procedure(Sender: TZNet; P_IO: TPeerIO)
    begin
      Cli1.Connect_P(procedure(const cState: Boolean)
        begin
          if cState then
              DoStatus('The construction of cli1 dual channel is completed');
        end);
      Cli2.Connect_P(procedure(const cState: Boolean)
        begin
          if cState then
              DoStatus('The construction of cli2 dual channel is completed');
        end);
      Cli3.Connect_P(procedure(const cState: Boolean)
        begin
          if cState then
              DoStatus('The construction of cli3 dual channel is completed');
        end);
    end;
