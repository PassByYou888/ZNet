...
unit Z.diocp_core_rawWinSocket;

interface

uses
  windows, SysUtils, Z.diocp_winapi_winsock2, Z.diocp_sockets_utils;
...
begin
  ...
  Result := Z.diocp_winapi_winsock2.Bind(FSocketHandle, PSockAddr(@sockaddr[0]), lvSize) = 0;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Z.diocp_winapi_winsock2.shutdown(lvTempSocket, SD_BOTH);
    end;
...
begin
  ...
  Result := Z.diocp_winapi_winsock2.Connect(FSocketHandle, TSockAddr(sockaddr), sizeof(TSockAddrIn))  = 0;
end;
...
begin
  ...
  lvRet := Z.diocp_winapi_winsock2.Connect(FSocketHandle, TSockAddr(sockaddr), sizeof(TSockAddrIn));
  if lvRet = 0 then
  begin  // 连接成功
    lvFlags := 0;  // 非阻塞模式
    ioctlsocket(FSocketHandle, Longint(FIONBIO), lvFlags);
  ...
  begin
    ...
    lvRet := Z.diocp_winapi_winsock2.select(FSocketHandle, nil, @fs, nil, Timeptr);
...
begin
  ...
  Result := Z.diocp_winapi_winsock2.Listen(FSocketHandle, queueSize) = 0;
end;

function TRawSocket.PeekBuf(var data; const len: Integer): Integer;
begin
  Result := Z.diocp_winapi_winsock2.recv(FSocketHandle, data, len, MSG_PEEK);
end;
...
begin
  ...
  Result := Z.diocp_winapi_winsock2.select(FSocketHandle + 1, ReadFdsptr, WriteFdsptr, ExceptFdsptr, Timeptr);
...
begin
  Result := Z.diocp_winapi_winsock2.Send(FSocketHandle, data, len, 0);
end;
...
begin
  Result := Z.diocp_winapi_winsock2.shutdown(FSocketHandle, pvHow);
end;
