...
uses
  Z.utils_strings, SysUtils, Z.utils_dvalue
...
{$ELSE}
{$IFDEF USE_ZLIBExGZ}
    Z.ZLibEx.ZCompress(pvBuilder.Memory, pvBuilder.length, lvOutBuf, lvOutBytes);
{$ELSE}
...
var
  ...
  l: Integer;
begin
  l := Z.utils_lzo.lzo_compressdestlen(pvBuilder.length);
  SetLength(lvOutBytes, l);
  l := Z.utils_lzo.lzo_compress(PAnsiChar(pvBuilder.Memory), pvBuilder.length,
...
var
  ...
  l: Integer;
begin
  l := Z.utils_lzo.lzo_decompressdestlen(PAnsiChar(pvBuilder.Memory));
  SetLength(lvOutBytes, l);
  l := Z.utils_lzo.lzo_decompress(PAnsiChar(pvBuilder.Memory), pvBuilder.length,
    PAnsiChar(@lvOutBytes[0]));
