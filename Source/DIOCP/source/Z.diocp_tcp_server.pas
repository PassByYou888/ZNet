...
uses
  Classes, Z.diocp_sockets_utils, Z.diocp_core_engine,
  winsock, Z.diocp_winapi_winsock2, Z.diocp_res,

{$if CompilerVersion >= 18}
  types,
{$ifend}

  Z.diocp_core_rawWinSocket, SyncObjs, Windows, SysUtils,
  Z.utils_safeLogger,
  Z.utils_hashs,
  Z.utils_queues, Z.utils_locker, Z.utils_strings, Z.utils_threadinfo
...
type
  ...
  private
    ...
    FCounter:Integer;
    FDebugInfo:String;
    FInnerBuffer: Z.diocp_winapi_winsock2.TWsaBuf;
    FRecvBuffer: Z.diocp_winapi_winsock2.TWsaBuf;
    FRecvdFlag: Cardinal;
    FOwner: TDiocpTcpServer;
...
begin
  //FContextLocker.lock();
  Z.utils_strings.SpinLock(FInnerLockerFlag);
end;

procedure TIocpClientContext.InnerUnLock;
begin
  //FContextLocker.unLock;
  Z.utils_strings.SpinUnLock(FInnerLockerFlag);
end;
...
begin
  ...
  begin
    ...
    raise Exception.CreateFmt('%s=>(%d,%d)当前对象已经被其他线程正在使用',
       [pvDebugInfo, Z.utils_strings.GetCurrentThreadID, FCheckThreadId]);
  end;
  FCheckThreadId := Z.utils_strings.GetCurrentThreadID;
...
begin
  ...
  begin
    ...
    lvRet := Z.diocp_winapi_winsock2.WSARecv(FClientContext.FRawSocket.SocketHandle,
       @FRecvBuffer,
       1,
