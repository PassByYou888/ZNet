...
uses
  Classes, StrUtils, SysUtils, Z.SHA, Z.utils_strings 

  {$IFDEF QDAC_QWorker}, qworker{$ENDIF}
  {$IFDEF DIOCP_Task}, Z.diocp_task{$ENDIF}
  , Z.diocp_tcp_server, Z.utils_queues, Z.utils_hashs, Z.utils_dvalue
  , Z.diocp_ex_http_common
  , Z.diocp_res
  , Z.utils_objectPool, Z.utils_safeLogger, Windows, Z.utils_threadinfo, SyncObjs,
  Z.utils_BufferPool,  Z.utils_websocket,  Z.utils_base64, DateUtils;
...
type
  ...
  TDiocpHttpCookie = Z.diocp_ex_http_common.TDHttpCookie;
...
begin
  ...
  begin
    ...
    raise Exception.CreateFmt('(%d,%d)当前对象已经被其他线程正在使用::%s',
       [Z.utils_strings.GetCurrentThreadID, FThreadID, s]);
  end;
  FThreadID := Z.utils_strings.GetCurrentThreadID;
end;
...
var
  ...
  s:string;
begin
  lvThreadID := Z.utils_strings.GetCurrentThreadID;
