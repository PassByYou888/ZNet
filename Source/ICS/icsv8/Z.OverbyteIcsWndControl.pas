...
{$ELSE MSWINDOWS}
    ...
    FHandle := Z.Ics.Posix.Messages.AllocateHWND(WndProc);
    AllocateMsgHandlers;
{$ENDIF}
end;
...
{$ELSE MSWINDOWS}
    FreeMsgHandlers;
    Z.Ics.Posix.Messages.DestroyWindow(FHandle);
    FHandle := 0;
        ...
        with MsgRec do begin
          ...
          {$ELSE} //Posix
                ...
                if { (not IsBadReadPtr(Obj, GUIDOffSet + SizeOf(INT_PTR))) and }
                   (PUINT_PTR(WParam + Z.Ics.Posix.Messages.WPARAM(GUIDOffSet))^ = WParam) and
                ...
                Obj := TObject(WParam);
                if {(not IsBadReadPtr(Obj, GUIDOffSet + SizeOf(INT_PTR))) and }
                   (PINT_PTR(WParam + Z.Ics.Posix.Messages.WPARAM(GUIDOffSet))^ = LParam) and
