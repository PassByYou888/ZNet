...
unit Z.ICS9.OverbyteIcsWndControl;
{$ENDIF}
...
{$ELSE MSWINDOWS}
    ...
    FHandle := Z.ICS9.Ics.Posix.PXMessages.AllocateHWND(WndProc);  { V8.65 }
    AllocateMsgHandlers;
{$ENDIF}
end;
...
{$ELSE MSWINDOWS}
    FreeMsgHandlers;
    Z.ICS9.Ics.Posix.PXMessages.DestroyWindow(FHandle);  { V8.65 }
    FHandle := 0;
        ...
        with MsgRec do begin
          ...
          {$ELSE} //Posix
                ...
                if { (not IsBadReadPtr(Obj, GUIDOffSet + SizeOf(INT_PTR))) and }
                   (PUINT_PTR(WParam + Z.ICS9.Ics.Posix.PXMessages.WPARAM(GUIDOffSet))^ = WParam) and { V8.65 }
                ...
                Obj := TObject(WParam);
                if {(not IsBadReadPtr(Obj, GUIDOffSet + SizeOf(INT_PTR))) and }
                   (PINT_PTR(WParam + Z.ICS9.Ics.Posix.PXMessages.WPARAM(GUIDOffSet))^ = LParam) and  { V8.65 }
